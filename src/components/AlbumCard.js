...
import modAlbumUrl from '../helpers/appleKit/modAlbumUrl'
...
let AlbumCard = ({ album }) => {
    ...
    useEffect(() => {
        let url = modUrl(album.attributes.artwork.url, 500, 500)
        setUrl(url)
    }, [album])
        ...
        <>
            <div id='card' className="flex flex-row mt-3 ml-3 items-center between">
                ...
                <div id='card-right' className='flex flex-col truncate w-2/3'>
                    <div className='font-bold '>{album.attributes.name}</div>
                    <div className='font-light'>{album.attributes.albumName}</div>
                </div>
            </div>
