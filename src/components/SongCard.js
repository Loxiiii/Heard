...
let SongCard = ({ song }) => {
    ...
    let songRoute = useRef('')

    useEffect(() => {
        let url = modUrl(song.attributes.artwork.url, 500, 500)
        setUrl(url)
        songRoute = `/song/${song.id}`
            ...
            <div id='card' className="flex flex-row mt-3 ml-3 items-center between">
                ...
                <div id='card-right' className='flex flex-col truncate w-2/3'>
                    <div className='font-bold '>
                        <Link href={`/song/${encodeURIComponent(song.id)}`}>{song.attributes.name}</Link>
                    </div>
                    <div className='font-light'>{song.attributes.artistName}</div>
                </div>
            </div>
